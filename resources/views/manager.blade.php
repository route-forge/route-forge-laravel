...
<script>
  ...
  let S = { tab: 'overview', q: '', tf: null, mf: null };
  ...
  function initTabs() {
    document.querySelectorAll('.tab').forEach(t => t.addEventListener('click', () => swTab(t.dataset.tab)));
  }

  function swTab(tab) {
    S.tab = tab;
    document.querySelectorAll('.tab').forEach(t => t.classList.toggle('on', t.dataset.tab === tab));
    document.querySelectorAll('.pn').forEach(p => p.classList.toggle('on', p.id === 'p-' + tab));
  }

  function renderOV() {
    const c = document.getElementById('tier-cards');
    ...
    all.forEach(n => {
      ...
      const cls = TC[n] ? 'c-' + n : 'c-def';
      h += `<div class="card ${cls}" onclick="filterT('${n}')"><div class="c-name">${esc(n)}</div><div class="c-desc">${esc(desc)}</div><div class="c-cnt">${cnt}</div>${load ? `<div class="c-meta">${load}</div>` : ''}</div>`;
    });
    c.innerHTML = h;
    const ms = {};
    D.routes.forEach(r => r.methods.forEach(m => {
      ms[m] = (ms[m] || 0) + 1;
    }));
    const sh = Object.entries(ms).map(([m, c]) => `<span class="mb m-${m}">${m}</span> ${c}`).join('&nbsp;&nbsp;&nbsp;');
    document.getElementById('ov-stats').innerHTML = `<div style="background:var(--sf);border:1px solid var(--bd);border-radius:var(--r);padding:16px 20px;font-size:14px"><strong>HTTP 方法分布：</strong>&nbsp;&nbsp;${sh}</div>`;
  }

  function renderTF() {
    const a = ['all', ...CFG.tiers.map(t => t.name)];
    if (D.tierCounts['unassigned']) a.push('unassigned');
    document.getElementById('tf').innerHTML = a.map(t => `<button class="fb${t === 'all' ? ' on' : ''}" data-t="${t}">${t === 'all' ? '全部' : t}</button>`).join('');
  }

  function renderRT() {
    const f = filtered();
    const tb = document.getElementById('rtb');
    document.getElementById('rc').textContent = f.length + ' 条路由';
    if (!f.length) {
      tb.innerHTML = '<tr><td colspan="5" class="emp">没有匹配的路由</td></tr>';
      return;
    }
    tb.innerHTML = f.map(r => {
      const tcls = TC[r.tier] ? 't-' + r.tier : 't-def';
      const ms = r.methods.map(m => `<span class="mb m-${m}">${m}</span>`).join('');
      const mw = r.middleware.length ? `<div class="mwl">${r.middleware.map(m => `<span class="mwt">${esc(m)}</span>`).join('')}</div>` : '<span style="color:var(--tx2)">—</span>';
      return `<tr><td><span class="rn" onclick="showD('${esc(r.name)}')">${esc(r.name)}</span></td><td><span class="tb ${tcls}">${r.tier}</span></td><td>${ms}</td><td class="ru">${esc(r.uri)}</td><td>${mw}</td></tr>`;
    }).join('');
  }

  function filtered() {
    return D.routes.filter(r => {
      if (S.tf && r.tier !== S.tf) return false;
      if (S.mf && !r.methods.includes(S.mf)) return false;
      if (S.q) {
        const q = S.q.toLowerCase();
        if (!(r.name + ' ' + r.uri + ' ' + r.middleware.join(' ')).toLowerCase().includes(q)) return false;
      }
      return true;
    });
  }
  ...
  function filterT(t) {
    ...
    swTab('routes');
    renderTF();
    document.querySelectorAll('#tf .fb').forEach(b => b.classList.toggle('on', b.dataset.t === (t || 'all')));
    renderRT();
  }
  ...
  function showD(name) {
    ...
    document.getElementById('mt').textContent = r.name;
    document.getElementById('mbd').innerHTML = `
    <div class="dr"><div class="dl">层级</div><div><span class="tb ${tcls}">${r.tier}</span></div></div>
    <div class="dr"><div class="dl">URI</div><div class="dv">${esc(r.uri)}</div></div>
    <div class="dr"><div class="dl">HTTP 方法</div><div>${ms}</div></div>
    <div class="dr"><div class="dl">路径参数</div><div>${ps}</div></div>
    <div class="dr"><div class="dl">中间件</div><div class="mwl">${mw}</div></div>`;
  ...
  function renderCFG() {
    ...
    document.getElementById('gf').innerHTML = fields.map(f => {
      if (f.t === 'check') return `<div class="fd fdc"><input type="checkbox" id="cf-${f.k}" data-k="${f.k}" ${f.v ? 'checked' : ''}><label for="cf-${f.k}">${f.l}</label></div>`;
      return `<div class="fd"><label>${f.l}</label><input type="${f.t}" id="cf-${f.k}" data-k="${f.k}" value="${esc(String(f.v))}"></div>`;
    }).join('');
    document.getElementById('le').value = JSON.stringify(CFG.levelsConfig, null, 2);
  }
  ...
  function bindEvts() {
    ...
    document.querySelectorAll('#tf').forEach(c => c.addEventListener('click', e => {
      if (!e.target.classList.contains('fb')) return;
      const t = e.target.dataset.t;
      S.tf = t === 'all' ? null : t;
      document.querySelectorAll('#tf .fb').forEach(b => b.classList.toggle('on', b === e.target));
      renderRT();
    }));
    document.querySelectorAll('#mf .fb').forEach(b => b.addEventListener('click', () => {
      const m = b.dataset.m;
      const wasOn = b.classList.contains('on');
      S.mf = wasOn ? null : m;
      document.querySelectorAll('#mf .fb').forEach(x => x.classList.remove('on'));
      if (!wasOn) b.classList.add('on');
      renderRT();
    }));
    document.getElementById('mc').addEventListener('click', () => document.getElementById('modal').classList.remove('show'));
    document.getElementById('modal').addEventListener('click', e => {
      if (e.target === e.currentTarget) e.currentTarget.classList.remove('show');
    });
    document.getElementById('bsave').addEventListener('click', saveCfg);
    document.getElementById('breset').addEventListener('click', () => renderCFG());
    document.getElementById('le').addEventListener('input', () => {
  ...
  async function saveCfg() {
    ...
    const je = document.getElementById('jerr');
    let levels;
    try {
      levels = JSON.parse(le.value);
    } catch (e) {
      je.textContent = 'JSON 解析错误: ' + e.message;
      je.style.display = 'block';
      return;
    }
    const global = {};
    document.querySelectorAll('#gf input').forEach(inp => {
